...
if __name__ == "__main__":
    ...
    parser.add_argument('--algorithm', type=str, default="IBIRM")
    ...
    parser.add_argument('--holdout_fraction', "--hf", type=float, default=0.05) #### Thuan dang de 5%
    ...
    parser.add_argument('--mmd_lambda', type=float)
    parser.add_argument('--mmd_step', type=int, default=0)
    ...
    parser.add_argument("--test_val", default= 0) ### dang de False
    ...
    if args.hparams_seed == 0:
        ...
        hparams["mmd_lambda"] = args.mmd_lambda if args.mmd_lambda is not None else hparams["mmd_lambda"]
        hparams["mmd_penalty_anneal_iters"] = args.mmd_step if args.mmd_step is not None else hparams["mmd_penalty_anneal_iters"]
    ...
    #### This can be used for all IB-IRM, CEM-MMD, CEM-CORAL and IRM-MMD
    print("RRRRRRR Hyper sweep for IB-IRM")
    report_val_acc = 0
    report_hyper = {}
    args.algorithm = "IBIRM"
    hparams['mmd_penalty_anneal_iters'] = 0
    hparams['ib_penalty_anneal_iters'] = 0
    hparams["normalize"] = True
    hparams['irm_penalty_anneal_iters'] = 0
    for mmd_lambda in [0.1, 1, 10, 100, 1000]:
        hparams["mmd_lambda"] = mmd_lambda
        for ib_lambda in [0.1, 1, 10, 100, 1000]:  
            hparams["ib_lambda"] = ib_lambda
            hparams["class_condition"] = False ### k dung gia thiet trong IRM
            for irm_lambda in [0.1, 1, 10, 100, 1000]:
                hparams["irm_lambda"] = irm_lambda
                test_acc, test_std, val_acc, val_std = run(args, hparams, device)
                print("RRRRRRR mmd_lambda:{:9.2f} ib_lambda:{:9.2f}   irm_lambda:{:9.2f} " # 
                    "val acc: {:.2f} +- {:.2f} test acc: {:.2f} +- {:.2f}".format(
                    mmd_lambda, ib_lambda, irm_lambda, val_acc, val_std, test_acc, test_std)) # 
                ...
                if val_acc > report_val_acc:
                    ...
                    report_hyper = { "mmd_lambda": mmd_lambda, "ib_lambda": ib_lambda, "irm_lambda": irm_lambda} # 
...
##### Other algotihms: ERM, IB-ERM and IRM
